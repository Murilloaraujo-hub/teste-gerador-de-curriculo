...
# Configure CORS for specific Frontend domain or fallback
...
# Global Flask Error Handlers to prevent returning HTML default pages
...
def is_valid_youtube_url(url_str):
    """
    Strict validation of YouTube URLs to prevent SSRF and injection.
    """
    ...
    # Block local/private IPs and suspicious hostnames
    forbidden_keywords = [
...
def sanitize_filename(name):
    """
    Removes unsafe characters for file system safety.
    """
...
def root():
    """Root route returning JSON status."""
    return jsonify({
        "status": "ok",
...
def get_video_info():
    """Retrieves basic metadata for YouTube video."""
...
def convert_video():
    ...
    Accepts JSON: { "url": "https://...", "quality": 192 }
    Extracts audio stream with yt-dlp and encodes with FFmpeg to MP3.
    """
    ...
    # 2. Process in secure Temporary Directory
